...
const Hsciifontpicker = () => {
  ...
  const setBodyFont = (selected_hsciifont_name: string) => {
    document.body.classList.value = document.body.classList.value.replaceAll(
      /__className_\w+\s+antialiased/g,
      ''
    )
    document.body.classList.value = document.body.classList.value.replaceAll(
      /antialiased\s+__className_\w+/g,
      ''
    )
    ...
    <div>
      <Select onValueChange={handle_fitem_change}>
        <SelectTrigger className="mt-4 w-[180px]">
          {' '}
          <SelectValue placeholder="hscii_font=>select" />{' '}
        </SelectTrigger>
        <SelectContent>
